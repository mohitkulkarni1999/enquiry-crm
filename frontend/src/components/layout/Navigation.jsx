...
const Navigation = ({ collapsed = false, onToggle }) => {
  ...
  return (
    <>
      <aside className={`fixed left-0 top-0 h-full bg-white border-r border-gray-200 shadow-2xl z-50 overflow-y-auto transition-all duration-300 ${collapsed ? 'w-16' : 'w-72'
        }`}>
        {/* Header */}
        <div className={`border-b border-gray-200 bg-gradient-to-r from-blue-600 via-blue-700 to-indigo-700 ${collapsed ? 'px-2 py-4' : 'px-6 py-6'
          }`}>
        ...
        {/* User Info */}
        {user && (
          <div className={`border-b border-gray-200 bg-gradient-to-r from-gray-50 to-blue-50 ${collapsed ? 'px-2 py-3' : 'px-6 py-5'
            }`}>
            <div className={`flex items-center ${collapsed ? 'justify-center' : 'space-x-3'}`}>
              <div className={`bg-gradient-to-r from-blue-500 via-purple-500 to-indigo-500 rounded-xl flex items-center justify-center text-white font-bold shadow-lg ${collapsed ? 'w-8 h-8 text-sm' : 'w-12 h-12 text-lg'
      ...
      <button
        ...
        className={`fixed top-1/2 transform -translate-y-1/2 bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-r-lg shadow-lg z-50 transition-all duration-300 ${collapsed ? 'left-16' : 'left-72'
          }`}
